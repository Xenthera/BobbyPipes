...
public final class ParcelRenderer {
    ...
    public static void extract(ExtractLevelRenderStateEvent event) {
        ...
        for (ClientParcels.Drawn entry : entries) {
            ...
            // Before the model resolve, which is the expensive part of this loop: a parcel
            // inside a chameleon-covered run cannot be seen, so none of the work below is
            // worth doing for it.
            if (isHidden(event.getLevel(), entry)) {
                continue;
            }
    ...
    /**
     * Whether this parcel is sealed inside covered pipe for the whole of its hop.
     *
     * <p>Both ends have to be covered. A parcel on its way out of a covered run into open
     * pipe is drawn for that last leg, so it emerges smoothly rather than appearing out of
     * nowhere at the mouth.
     *
     * <p>Reads the block state rather than the block entity: the cover flag lives on the
     * state precisely so checks like this stay cheap.
     */
    private static boolean isHidden(BlockGetter level, ClientParcels.Drawn entry) {
        // Goggles are how you watch traffic through a covered run, so they suspend the
        // optimisation as well as the disguise. Wearing them costs what covers save.
        if (ClientPipeCovers.seeThroughCovers()) {
            return false;
        }
        if (!isCovered(level, entry.at())) {
            return false;
        }
        return entry.next().map(next -> isCovered(level, next)).orElse(true);
    }

    private static boolean isCovered(BlockGetter level, BlockPos pos) {
        BlockState state = level.getBlockState(pos);
        return state.getBlock() instanceof PipeBlock && state.getValue(PipeBlock.COVERED);
    }
